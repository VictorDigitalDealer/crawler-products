...
export async function crawlCategory(category) {
    ...
    while (pageUrl && page <= maxPagesSafeGuard) {
        ...
        try {
            ...
            const { products, pagination } = normalizeParseResult(parseResult.products);
            ...
            console.error(`Error al procesar la categoría ${category.name} (${category.shopId}) en página ${page}:`, getErrorMessage(err));
            break;
        }
    }
