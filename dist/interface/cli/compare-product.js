...
async function runComparison(productId, shopName) {
    try {
        const referenceProduct = await getProductById(productId);
        if (!referenceProduct) {
            console.log(`No se encontró ningún producto con id ${productId}.`);
            return;
        }
        console.log(`Producto de referencia (id=${referenceProduct.id}): ${referenceProduct.name} [${referenceProduct.shop}]`);
        const productsFromShop = await getProductsByShop(shopName);
        if (productsFromShop.length === 0) {
            console.log(`No se encontraron productos para la tienda ${shopName}.`);
            return;
        }
        console.log(`Productos encontrados en la tienda ${shopName}: ${productsFromShop.length}`);
        const candidates = productsFromShop.filter((p) => p.id !== referenceProduct.id);
        if (candidates.length === 0) {
            console.log("No hay candidatos para comparar (solo está el producto de referencia).");
            return;
        }
        const comparisonResults = await Promise.all(candidates.map((candidate) => compareProducts(referenceProduct, candidate)));
        const comparisons = comparisonResults.filter((c) => c !== null);
        if (!comparisons.length) {
            console.log("No se encontraron coincidencias relevantes.");
            return;
        }
        console.log(`Comparaciones generadas: ${comparisons.length}. Guardando en la base de datos...`);
...
const shopNameArg = process.argv[3];
