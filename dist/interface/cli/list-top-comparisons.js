...
async function listTopComparisons(limit = 20) {
    try {
        ...
        comparisons.forEach((comp, index) => {
            ...
            console.log(`${index + 1}) ${comp.totalMatchPercentage.toFixed(2)}%  ` +
                `(nombre: ${comp.nameMatchPercentage.toFixed(2)}%, categoría: ${comp.categoryMatchPercentage.toFixed(2)}%, precio: ${comp.priceMatchPercentage.toFixed(2)}%)`);
            console.log(`   GCDS [id=${gc.id}, shop=${gc.shop}]  ` +
                `${gc.name}  |  ${gc.price.toFixed(2)}€`);
            console.log(`   URL GCDS: ${gc.url}`);
            console.log(`   EXT  [id=${ext.id}, shop=${ext.shop}]  ` +
        ...
        console.error("Error al listar las mejores comparaciones:", getErrorMessage(error));
    }
    finally {
...
if (Number.isNaN(limit) || limit <= 0) {
    console.log("Uso: pnpm run list:top-comparisons [limit]\n" +
